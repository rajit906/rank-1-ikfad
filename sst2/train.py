...
from optimizers import iKFAD, iKFAD_R1
...
def get_optimizer(model, optim_name, hp, device):
    ...
    if optim_name == 'adam':
        ...
        return iKFAD(params, h=hp['h'], gamma=hp['gamma'], alpha=hp['alpha'], mu=hp['mu'], device=device)
    elif optim_name == 'rikfad0':
        print('h: ', hp['h'], 'gamma: ', 0., 'alpha: ', hp['alpha'], 'mu: ', hp['mu'])
        return iKFAD_R1(params, h=hp['h'], gamma=0., alpha=hp['alpha'], mu=hp['mu'])
    elif optim_name == 'adafactor':
        print('lr: ', hp['lr'], 'beta1: ', hp.get('beta1', None))
        return Adafactor(params, lr=hp['lr'], beta1=hp.get('beta1', None),
...
def train_engine(optim_name, hp, num_epochs, device, metric, seed):
    ...
    # 2. Data & Model
    print(f"Loading data for SST2...", flush=True)
    train_loader, valid_loader, test_loader = get_dataloaders(batch_size=16)
    
    print(f"Initializing model for SST2...", flush=True)
    ...
    # Checkpoint setup
    checkpoint_dir = os.path.join('sst2', 'checkpoints', optim_name)
    ...
    for epoch in range(num_epochs):
        ...
        # 2. Save Best based on Validation Loss
...
if __name__ == "__main__":
    ...
    parser.add_argument('--optimizer', type=str, required=True, help='one of: adam, adafactor, ikfad, ikfad0, rikfad, rikfad0')
