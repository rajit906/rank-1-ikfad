...
def run_single_seed(optim_name, hp, device, seed, num_epochs=2):
    ...
    # 2. Data 
    train_loader, _, test_loader = get_dataloaders(batch_size=16)
    ...
    # Calculate interval to hit exactly 10x per epoch
    eval_interval = int(total_batches / 10)
    if eval_interval < 1: 
        eval_interval = 1
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # --- Evaluation Logic: 10x per epoch ---
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='SST2 Multi-Seed Runner')
