...
def get_lr(it, config):
    ...
    warmup = config.get('warmup_iters', 0)
    decay_iters = config.get('lr_decay_iters', config['max_iters'])
    if not config.get('decay_lr', False):
        return max_lr
    if it < warmup:
        return max_lr * it / max(warmup, 1)
    if it > decay_iters:
        return min_lr
...
class GPTTrainer:
    ...
    def train(self):
        ...
        while True:
            ...
            # LR schedule
            if self.config.get('decay_lr', False):
                lr = get_lr(iter_num, self.config)
                opt_name = self.config.get('optimizer_name', 'ADAM')
                if opt_name in ('ADAM', 'mSGD'):
                    for pg in self.optimizer.param_groups:
                        pg['lr'] = lr
                else:
                    h_base = self.config.get('optimizer_params', {}).get('h', 0.001)
                    schedule_factor = lr / self.config['learning_rate']
                    for pg in self.optimizer.param_groups:
                        pg['h'] = torch.tensor([h_base * schedule_factor], device=self.device)
