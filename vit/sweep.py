...
def get_hp_from_trial(trial, optimizer_name):
    ...
    # Defaults
    defaults = {'lr': 0.0, 'h': 0.0, 'momentum': 0.0, 'betas': (0.9, 0.999),
                'gamma': 0.0, 'c': 0.0, 'alpha': 0.0, 'mu': 0.0, 'beta1': None}
    hp.update(defaults)
    ...
    elif optimizer_name == 'rikfad0':
        p = rikfad0_params(trial)
        hp.update(p)

    elif optimizer_name == 'rikfad':
        p = rikfad_params(trial)
        hp.update(p)

    elif optimizer_name == 'adafactor':
        p = adafactor_params(trial)
        hp['lr'] = p['lr']
        hp['beta1'] = p['beta1']
...
def objective(trial, args):
    ...
    try:
        # train_engine returns best_val_loss
        result = train_engine(
            optim_name=args.optimizer,
            hp=hp,
            num_epochs=args.epochs,
            device=args.device,
            metric='loss', # This is passed but train.py is hardcoded to return val_loss per request
            seed=args.seed
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='CIFAR10 Optuna Sweep')
    parser.add_argument('--optimizer', type=str, required=True, 
                        choices=['adam', 'adafactor', 'ikfad', 'ikfad0', 'rikfad', 'rikfad0'],
                        help='Optimizer to sweep')
    parser.add_argument('--n_trials', type=int, default=50, help='Number of Optuna trials')
    parser.add_argument('--epochs', type=int, default=10, help='Epochs per trial (Recommend 10+ for ViT)')
